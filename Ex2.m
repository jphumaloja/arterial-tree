...
% aux functions
gs = d0.*subs((us+vs).*(us-vs).^4,x,1);
gsu = d0.*subs((5*us+3*vs).*(us-vs).^3,x,1);
gsv = -d0.*subs((3*us+5*vs).*(us-vs).^3,x,1);
...
rod = -gsu./gsv; fod = 1./gsv; qd = -Gsv./Gsu;
...
fd = fod.*exp(subs(S22./mud,x,1)).*(0.1*ones(1,m)); % *p^i percentages
% output distribution
gd = [0 0 0 1 2 4 3 0 0 0];
g1m = 4; g2m = 7;
...
[A, C, x0] = waveform('Q');
C(1) = C(1) + 2e-4; % adjust zero harmonic to get positive inflow
n = numel(C);
...
L = -lqr(A',GC',1e4*eye(n),1)'; % output injection gain L
...
% plant
um0 = @(x,i) -0.5*exp(-double(S11(i))/lamm(i)*(x+1)/2);
vm0 = @(x,i) 0.75*exp(double(S22(i))/mum(i)*(x+1)/2);
Ntm = 2*Nxm;
